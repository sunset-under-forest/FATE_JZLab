...
int32_t
read_test_options(int32_t *argcp, char ***argvp, e_role *role, uint32_t *bitlen, uint32_t *nvals, uint32_t *secparam,
...
e_role get_role(int32_t *argcp, char ***argvp) {
    ...
    read_test_options(argcp, argvp, &role, &bitlen, &nvals, &secparam, &address,
                      &port, &test_op);
...
share *BuildAddCircuit( share *s_x1, share *s_x2,BooleanCircuit *circ) {
    share *s_out = circ->PutADDGate(s_x1, s_x2);
    return s_out;
}
...
share *BuildMulCircuit(share *s_x1, share *s_x2,BooleanCircuit *circ) {
    share *s_out = circ->PutMULGate(s_x1, s_x2);
    return s_out;
}
