...
def create_and_get(job_id, idx, data):
    ...
    with ABY():
        if idx == 0:
            ...
            x = ABYFixedPointTensor.from_source("x", all_parties[0])
        return x.get()
...
def add_and_sub(job_id, idx, data_list):
    ...
    with ABY():
        if idx == 0:
            x = ABYFixedPointTensor.from_source("x", data_list[0])
            y = ABYFixedPointTensor.from_source("y", all_parties[1])
        else:
            x = ABYFixedPointTensor.from_source("x", all_parties[0])
            y = ABYFixedPointTensor.from_source("y", data_list[1])
        a = (x + y).get()
        b = (x - y).get()
        return a, b
