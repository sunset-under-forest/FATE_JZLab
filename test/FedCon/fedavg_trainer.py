...
class FedAVGTrainer(TrainerBase):
    ...
    def __init__(self, epochs=10, batch_size=512,  # training parameter
        ...
        self.check_trainer_param(
            [self.secure_aggregate, self.weighted_aggregation, self.pin_memory, self.save_to_local_dir], [
                'secure_aggregate', 'weighted_aggregation', 'pin_memory', 'save_to_local_dir'], self.is_bool,
            '{} is not a bool')
        self.check_trainer_param(
    ...
    def update_torch_adacon(self, model, param: HomoLogisticParam, contribution):
        LOGGER.info("--------------Update a new optimizer.---------------------")

        learning_rate = param.learning_rate
        alpha = contribution * contribution * contribution
        decay = param.decay
        decay_sqrt = param.decay_sqrt

        if not decay_sqrt:
            def decay_func(epoch):
                return 1 / (1 + epoch * decay)
        else:
            def decay_func(epoch):
                return 1 / math.sqrt(1 + epoch * decay)

        LOGGER.info('alpha is {}'.format(alpha))
        optimizer = t.optim.Adagrad(params=model.parameters(), lr=learning_rate, weight_decay=alpha)

        LOGGER.info("--------------Update a new optimizer success.---------------------")

        return optimizer

    def train_an_epoch(self, epoch_idx, model, train_set, optimizer, loss, contribution):

        epoch_loss = 0.0
        batch_idx = 0
        acc_num = 0

        optimizer = self.update_torch_adacon(model, self.model_param, contribution)

        LOGGER.info("optimizer type is {}".format(type(optimizer).__name__))
        LOGGER.info("Current weight decay: " + str(optimizer.param_groups[0]['weight_decay']))
    ...
    def train(
        ...
        for i in range(self.epochs):
            ...
            LOGGER.info('client current contribution is {}'.format(contribution))
            model = self._select_model()
            epoch_loss = self.train_an_epoch(i, model, train_set, optimizer, loss, contribution)
            if not distributed_util.is_distributed() or distributed_util.is_rank_0():
            ...
            if client_agg is not None or distributed_util.is_distributed():
                if not (self.aggregate_every_n_epoch is not None and (i + 1) % self.aggregate_every_n_epoch != 0):
                    ...
                    if not distributed_util.is_distributed() or distributed_util.is_rank_0():
                        self.individual_contribution = client_agg.get_individual_contribution(epoch_loss)
                        # LOGGER.info('individual_contribution is {}'.format(self.individual_contribution))
                        # total_contribution = client_agg.get_total_contribution(self.individual_contribution)
                        # LOGGER.info('total_contribution is {}'.format(total_contribution))
                        self.model = client_agg.model_aggregation(self.model, self.individual_contribution)
                        LOGGER.info('Client model_aggregation success.Weight = {}'.format(client_agg._weight))
                        contribution = client_agg._weight
